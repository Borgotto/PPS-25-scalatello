...
class BoardTest extends AnyFlatSpec:
  ...
  private val topLeftCenterPos: Position = Position(BOARD_SIZE / 2 - DIST, BOARD_SIZE / 2 - DIST)
  ...
  private val validMovePos: Position = Position(topLeftCenterPos.row, topLeftCenterPos.column + DIST + DIST)
  private val notValidMovePos: Position = Position(topLeftCenterPos.row + DIST, topLeftCenterPos.column - DIST)
  ...
  "A Square Board" should "be able to place a new disk in a valid position" in :
    ...
    val expectedBoard: Board = Board(BOARD_SHAPE, boardDuringGame.disks + (validMovePos -> blackDisk))
  ...
  "A Square Board, if the move is not valid" should "not place the new disk" in :
    ...
    val expectedBoard: Board = Board(BOARD_SHAPE, boardDuringGame.disks)
    board.equals(expectedBoard) should be(true)
