# Documentazione di processo

## Sprint backlog

Il backlog degli sprint è visualizzabile alla [seguente pagina](https://github.com/users/Borgotto/projects/1); per ciascuno sprint, sono elencati i task in esso inclusi, esplicitandone priorità, dimensione, stato e assegnatario.

## Sprint report

Per ciascuno sprint, è disponibile un report che ne documenta obiettivi, svolgimento e risultati.

- [Sprint 0](./0-sprint.md)
- [Sprint 1](./1-sprint.md)
- [Sprint 2](./2-sprint.md)
- [Sprint 3](./3-sprint.md)
- [Sprint 4](./4-sprint.md)
- [Sprint 5](./5-sprint.md)
- [Sprint 6](./6-sprint.md)
